"""Manage system cron job for scheduled trading cycles (kodiak run-once).

Used by `kodiak schedule enable` and `kodiak schedule disable`.
...
def get_kodiak_path() -> str | None:
    """Return full path to 'kodiak' executable, or None if not on PATH."""
    return shutil.which("kodiak")
...
def _build_cron_line(kodiak_path: str, every_minutes: int) -> str:
    ...
    return f"{spec} {kodiak_path} run-once   {AUTOTRADER_MARKER}"
...
def get_schedule_status() -> dict:
    """Return status dict: enabled, line (if enabled), kodiak_path, supported."""
    supported = _is_unix()
    kodiak_path = get_kodiak_path() if supported else None
    ...
    return {
        ...
        "kodiak_path": kodiak_path,
        "supported": supported,
    }


def enable_schedule(every_minutes: int = 5) -> None:
    """Add a cron job that runs 'kodiak run-once' every every_minutes minutes.

    Raises:
        RuntimeError: If not on Unix or 'kodiak' not on PATH.
        ValueError: If every_minutes not in 1..60.
    """
    if not _is_unix():
        raise RuntimeError("Schedule (cron) is only supported on macOS and Linux.")
    path = get_kodiak_path()
    if not path:
        raise RuntimeError(
            "'kodiak' not found on PATH. Install with pipx or ensure the kodiak binary is on PATH."
        )
